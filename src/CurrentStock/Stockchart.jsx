  ...
  if (active && payload?.length) {
    return (
      <div className="custom-tooltip">
        ...
        <p className="stock">Stock: {payload[0].value} kg</p>
      </div>
    );
  }
  return null;
...
// Formatter for labels to show 2 decimal places
...
const Stockchart = ({ data, selectedDate }) => {
  ...
  const barColor = '#4a90e2'; // Use your desired single color
              ...
              <tbody>
                {data.map((entry, index) => (
                  <tr key={entry.name} style={{ '--row-index': index }}>
                    ...
                    <td>{Number(entry.stock).toFixed(2)} kg</td>
                  </tr>
                ))}
              </tbody>
            </table>
